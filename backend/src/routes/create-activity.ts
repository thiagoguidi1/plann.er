...
export async function createActivity(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().post(
    ...
    if (dayjs(occurs_at).isBefore(trip.starts_at)) {
      throw new Error('Invalid activity date, the activity date must be after the trip creation date.')
    }
    //Se a data da atividade for depois da data de termino da trip, retorna um erro
    if (dayjs(occurs_at).isAfter(trip.ends_at)) {
      throw new Error('Invalid activity date, the date of the activity must be before the end date of the trip.')
    }
